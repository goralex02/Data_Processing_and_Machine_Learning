function Gorbuntsov_Lab1()
    ...
    title('Optimized by 2-opt swap route')
end
...
function [ seq ] = GenerateStateCandidate2opt(seq) %changed function
    n = size(seq, 1); % get size of cities indexes array
    i = randi(n, 1); % get a pseudorandom index between 1 and n.
    j = randi(n, 1);
    ...
    dist=l_city - f_city; % distance between cities, how many cities should we visit
    dist=floor((dist+1)/2); % how many pairs of cities should we swap
    for a = 0:dist-1
        t = seq(f_city+a);
        seq(f_city+a) = seq(l_city-a);
        seq(l_city-a) = t;
    end
